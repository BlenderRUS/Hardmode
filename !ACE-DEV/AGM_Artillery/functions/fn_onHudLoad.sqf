...
_this spawn {
  ...
  waitUntil {
    if ((isNull _display) || {(vehicle ACE_player) != _mortarVeh}) exitWith {true};
    //Compute: Current Tube Settings (from animationPhase)
//    _elevationDegrees = ((180 / PI) * (_mortarVeh animationPhase "mainGun")) + 75;
    _elevationDegrees = (90 - (acos ((_mortarVeh) weaponDirection (currentWeapon (_mortarveh))  select 2)));
    ...
    _chargeText = format ["%1: %2<img image='\x\a3ru\addons\AGM_Artillery\UI\ui_charges.paa'/>", (toUpper localize "STR_AGM_Artillery_Charge"), _currentChargeMode];
    ...
    if (_digialDisplayEnabled) then {
		...
		if (((_mortarVeh) iskindof "RDS_2b14_82mm_Base") || ((_mortarVeh) iskindof "RDS_AGS_base") || ((_mortarVeh) iskindof "RDS_D30_base")) then { 
			_elevationString = [_elevationDegrees, "milru", true] call AGM_Artillery_fnc_formatNumber;
		} else {
			_elevationString = [_elevationDegrees, "mil4", true] call AGM_Artillery_fnc_formatNumber; 
		};
		_elevText = _elevText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/>", (_elevationString select [0, 1])];
		_elevText = _elevText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/>", (_elevationString select [1, 1])];
		_elevText = _elevText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/>", (_elevationString select [2, 1])];
		_elevText = _elevText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/></t></t>", (_elevationString select [3, 1])];
    };
    ...
    if (_digialDisplayEnabled) then {
      ...
      _aziText = _aziText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/>", (_azimuthString select [0, 1])];
      _aziText = _aziText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/>", (_azimuthString select [1, 1])];
      _aziText = _aziText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/>", (_azimuthString select [2, 1])];
      _aziText = _aziText + format ["<img image='\x\a3ru\addons\AGM_Artillery\rsc\vector_%1.paa'/></t></t>", (_azimuthString select [3, 1])];
    };
