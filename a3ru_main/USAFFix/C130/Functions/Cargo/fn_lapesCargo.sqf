...
_obj attachTo [_carrier,[0,-11,(-5.55 + (boundingBoxReal _obj select 1 select 2))]];
sleep 0.1;
detach _obj;
...
sleep 1;
//if (isServer) then {
    _pls = [_carrier,_obj] call USAF_C130_fnc_prepareForDrop;
    ...
    [_obj] spawn USAF_C130_fnc_applyBrakes;
