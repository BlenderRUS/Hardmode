...
if (count _cargos <= 0) exitWith { hint "Found nothing to do LAPES with." };
...
_carrier setVariable ["c130_cargo_loading",false,true];
    ...
    (driver _carrier) vehicleChat format ["%1 LAPES from cargo",_veh_name];
	[[_carrier, _cargo], "USAF_C130_fnc_lapesCargo"] spawn BIS_fnc_MP;
