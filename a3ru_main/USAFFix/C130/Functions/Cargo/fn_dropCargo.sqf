...
//};
sleep 0.5;
waitUntil {getPos _obj select 2 < 300};

//if (isServer) then {
    _pls = [(driver _carrier)] call USAF_C130_fnc_prepareForDrop;
    _chute = _pls select 0;
    _strobe = _pls select 1;
    _smoke = _pls select 2;
    [[_obj,_smoke,{(getPos (_this select 0) select 2 < 300 && getPos (_this select 0) select 2 > 1) && !(count crew (_this select 0) > 0)}], "USAF_C130_fnc_smoke"] spawn BIS_fnc_MP;
    ...
    waitUntil {(getPos _obj select 2) < 1 || isNull (_obj) || isNull (_chute)};
    detach _obj;
